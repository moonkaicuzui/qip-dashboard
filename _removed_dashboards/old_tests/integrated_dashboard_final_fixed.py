...
<head>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead class="unified-table-header">
                                <tr>
                                    <th class="sortable-header ${sortColumn === 'empNo' ? sortOrder : ''}" onclick="window.zeroModalSort('empNo')">${{translations.common?.tableHeaders?.employeeNo?.[lang] || '사번'}}</th>
                                    <th class="sortable-header ${sortColumn === 'name' ? sortOrder : ''}" onclick="window.zeroModalSort('name')">${{translations.common?.tableHeaders?.name?.[lang] || tr('orgChartModal.name', '이름')}}</th>
                                    <th class="sortable-header ${sortColumn === 'position' ? sortOrder : ''}" onclick="window.zeroModalSort('position')">${{translations.common?.tableHeaders?.position?.[lang] || '직책'}}</th>
            ...
            <div class="modal-body">
                ...
                <div class="table-responsive">
                    <table class="table table-hover" id="minimumDaysTable" style="font-size: 14px;">
                        <thead class="unified-table-header">
                            <tr>
                                <th class="sortable-header" data-sort="empNo" onclick="window.minDaysSort('empNo')" style="min-width: 100px;">${{translations.common?.tableHeaders?.employeeNo?.[lang] || '사번'}}</th>
                                <th class="sortable-header" data-sort="name" onclick="window.minDaysSort('name')" style="min-width: 130px;">${{translations.common?.tableHeaders?.name?.[lang] || tr('orgChartModal.name', '이름')}}</th>
                                <th class="sortable-header" data-sort="position" onclick="window.minDaysSort('position')" style="min-width: 150px;">${{translations.common?.tableHeaders?.position?.[lang] || '직책'}}</th>
        ...
        function updateTableBody() {
            ...
            below88Employees.forEach(emp => {
                ...
                row.innerHTML = `
                    ...
                    <td style="padding: 10px;"><span class="badge ${attendanceRate < 88 ? 'bg-danger' : 'bg-success'}" style="font-size: 13px; padding: 4px 8px;">${attendanceRate < 88 ? ${{translations.modals?.fprs?.conditionNotMet?.[lang] || '미충족'}} : ${{translations.modals?.fprs?.met?.[lang] || '충족'}}}</span></td>
                `;
                        ...
                        <div class="modal-body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover" style="font-size: 14px;">
                                    <thead class="unified-table-header">
                                        <tr>
                                            <th class="sortable-header" data-sort="empNo" style="min-width: 100px; padding: 12px; cursor: pointer;">${{translations.common?.tableHeaders?.employeeNo?.[lang] || '사번'}} ${getSortIcon('empNo')}</th>
                                            <th class="sortable-header" data-sort="name" style="min-width: 130px; padding: 12px; cursor: pointer;">${{translations.common?.tableHeaders?.name?.[lang] || tr('orgChartModal.name', '이름')}} ${getSortIcon('name')}</th>
    ...
    function showConsecutiveAqlFailDetails() {
        ...
        if (threeMonthFails.length === 0) {
            ...
            modalHTML += '<th style="border: 1px solid #dee2e6; padding: 8px;">${{translations.common?.tableHeaders?.name?.[lang] || tr('orgChartModal.name', '이름')}}</th>';
            modalHTML += '<th style="border: 1px solid #dee2e6; padding: 8px;">${{translations.common?.tableHeaders?.position?.[lang] || '직책'}}</th>';
    ...
    function showAreaRejectRateDetails() {
        ...
        const modalContent = `
            ...
            <div class="modal-body">
                <div class="mb-3">
                    <div class="alert alert-info">
                        <strong>${{translations.modals?.areaAQL?.condition7?.[lang]?.split(':')[0] || '조건 7번'}}:</strong> ${{translations.modals?.areaAQL?.condition7?.[lang]?.split(': ')[1] || '팀/구역 AQL 3개월 연속 실패'}} - ${cond7FailEmployees.length}명<br>
                        <strong>${{translations.modals?.areaAQL?.condition8?.[lang]?.split(':')[0] || '조건 8번'}}:</strong> ${{translations.modals?.areaAQL?.condition8?.[lang]?.split(': ')[1] || '구역 Reject Rate 3% 초과'}} - ${cond8FailEmployees.length}명
                    </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered" style="font-size: 13px;">
                                        <thead class="table-light">
                                            <tr>
                                                <th style="padding: 10px;">${{translations.modals?.areaAQL?.area?.[lang] || '구역'}}</th>
                                                <th style="padding: 10px; text-align: center;">${{translations.modals?.areaAQL?.totalEmployees?.[lang] || '전체 인원'}}</th>
    ...
    function showLowPassRateDetails() {
        ...
        function updateTableBody() {
            ...
            lowPassEmployees.forEach(emp => {
                ...
                const name = emp['Full Name'] || emp['name'];
                // 직책 1/2/3단계 모두 표시
                const position1 = emp['QIP POSITION 1ST  NAME'] || '';
                const position2 = emp['QIP POSITION 2ND  NAME'] || '';
                const position3 = emp['QIP POSITION 3RD  NAME'] || '';
                const positionDisplay = `${position1}${position2 ? ' > ' + position2 : ''}${position3 ? ' > ' + position3 : ''}`;

                // 통과율 계산 근거 데이터
                const totalTests = parseInt(emp['5PRS_Inspection_Qty'] || emp['validation_qty'] || 0);
                const passTests = Math.round(totalTests * parseFloat(emp['pass_rate'] || emp['5PRS Pass Rate'] || 0) / 100);
                ...
                row.innerHTML = `
                    <td>${empNo}</td>
                    <td>${name}</td>
                    <td style="font-size: 12px;">${positionDisplay}</td>
                    <td>TYPE-1</td>
                    <td>${totalTests}족</td>
                    <td>${passTests}족</td>
                    <td><span class="badge ${badgeClass}">${passRate}%</span></td>
                    <td>${passRate < 95 ? '<span class="badge bg-danger">미충족</span>' : '<span class="badge bg-success">충족</span>'}</td>
                `;
                tbody.appendChild(row);
            });
        }
                        ...
                        <div class="modal-body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    <thead class="unified-table-header">
                                        <tr>
                                            <th class="sortable-header" data-sort="empNo">${{translations.common?.tableHeaders?.employeeNo?.[lang] || '사번'}} ${getSortIcon('empNo')}</th>
                                            <th class="sortable-header" data-sort="name">${{translations.common?.tableHeaders?.name?.[lang] || tr('orgChartModal.name', '이름')}} ${getSortIcon('name')}</th>
                                            <th class="sortable-header" data-sort="position">${{translations.modals?.fprs?.positionHierarchy?.[lang] || '직책 (1단계 > 2단계 > 3단계)'}} ${getSortIcon('position')}</th>
                                            <th>타입</th>
                                            <th>${{translations.modals?.fprs?.totalTests?.[lang] || '총 검증'}}</th>
                                            <th>PASS</th>
                                            <th class="sortable-header" data-sort="passRate">통과율 ${getSortIcon('passRate')}</th>
                                            <th>${{translations.modals?.fprs?.conditionMet?.[lang] || '조건 충족'}}</th>
                                        </tr>
                                    </thead>
                                    <tbody></tbody>
                                </table>
        ...
        function updateTableBody() {
            ...
            lowQtyEmployees.forEach(emp => {
                ...
                row.innerHTML = `
                    ...
                    <td>${inspectionQty < 100 ? '<span class="badge bg-danger">미충족</span>' : '<span class="badge bg-success">충족</span>'}</td>
                `;
                tbody.appendChild(row);
            });
        }
...
<body>
    <div class="container">
        ...
        <div class="content p-4">
            ...
            <div class="tabs">
                ...
                <div class="tab" data-tab="validation" onclick="showTab('validation')" id="tabValidation">${{translations.tabs?.validation?.[lang] || tr('tabs.validation', '요약 및 시스템 검증')}}</div>
            </div>
    ...
    html_content += f'''
            ...
            <div id="detail" class="tab-content">
                ...
                <div class="table-responsive">
                    <table class="table" id="employeeTable">
                        <thead>
                            <tr>
                                <th id="empIdHeader">${{translations.common?.tableHeaders?.employeeNo?.[lang] || '사번'}}</th>
                                <th id="nameHeader">${{translations.common?.tableHeaders?.name?.[lang] || tr('orgChartModal.name', '이름')}}</th>
                            ...
                            <tbody>
                                ...
                                <tr style="background-color: #fff3e0;">
                                    ...
                                    <td><span class="calc-example-employee" data-employee="618110077">예시: 618110077 직원</span><br>
                                        ...
                                        <span class="calc-total-label">${{translations.orgChartModal?.total?.[lang] || tr('orgChartModal.total', '합계')}}</span>: 2,600,000 VND</td>
                                </tr>
                    ...
                    <div class="card-body">
                        ...
                        <div id="orgChartContainer" class="collapsible-tree">
                            ...
                            <div class="org-header">
                                <h4 id="orgChartTitle">${{translations.orgChart?.type1ManagerStructure?.[lang] || tr('orgChart.type1ManagerStructure', 'TYPE-1 관리자 인센티브 구조')}}</h4>
        ...
        <div id="validation" class="tab-content">
            <h3 id="validationTabTitle">${{translations.tabs?.validation?.[lang] || tr('tabs.validation', '요약 및 시스템 검증')}}</h3>
            ...
            <div class="kpi-cards-grid">
                ...
                <div class="kpi-card" onclick="showValidationModal('minimumDaysNotMet')" style="--card-color-1: #95a5a6; --card-color-2: #7f8c8d; box-shadow: 0 4px 15px rgba(149, 165, 166, 0.1);">
                    ...
                    <div class="kpi-label">${{translations.validationTab?.kpiCards?.minWorkingDays?.title?.[lang] || '최소 근무일 미충족'}}</div>
                </div>
                ...
                <div class="kpi-card" onclick="showValidationModal('attendanceBelow88')" style="--card-color-1: #9b59b6; --card-color-2: #8e44ad; box-shadow: 0 4px 15px rgba(155, 89, 182, 0.1);">
                    ...
                    <div class="kpi-label">${{translations.validationTab?.kpiCards?.lowAttendance?.title?.[lang] || '출근율 88% 미만'}}</div>
                </div>
                ...
                <div class="kpi-card" onclick="showValidationModal('areaRejectRate')" style="--card-color-1: #3498db; --card-color-2: #2980b9; box-shadow: 0 4px 15px rgba(52, 152, 219, 0.1);">
                    ...
                    <div class="kpi-label">${{translations.validationTab?.kpiCards?.areaRejectRate?.title?.[lang] || '구역 AQL Reject 3% 이상'}}</div>
                </div>
    ...
    <script>
        ...
        function updateFAQQASection() {{
            ...
            if (answer8Reason2) {{
                answer8Reason2.innerHTML = `<strong>TYPE-2 ${{lang === 'ko' ? tr('orgChartModal.position', '직급') : lang === 'en' ? 'positions' : 'vị trí'}}</strong>: ${{translations.incentiveCalculation?.faq?.answer8Reason2?.[lang] || 'TYPE-1 평균값 변동'}}`;
            }}
        ...
        // 인센티브 ${{translations.orgChartModal?.receivingStatus?.[lang] || tr('orgChartModal.receivingStatus', '수령 여부')}} 확인 함수
        ...
        function getIncentiveFailureReasons(employee) {{
            ...
            if (employee['attendancy condition 1 - acctual working days is zero'] === 'yes') {{
                reasons.push('${{translations.orgChartModal?.nonPaymentReasons?.actualWorkingDays0?.[lang] || '실제 근무일 0일 (출근 조건 1번 미충족)'}}');
            }}
            if (employee['attendancy condition 2 - unapproved Absence Day is more than 2 days'] === 'yes') {{
                reasons.push('${{translations.orgChartModal?.nonPaymentReasons?.unauthorizedAbsence?.[lang] || '무단결근 2일 초과 (출근 조건 2번 미충족)'}}');
            }}
            if (employee['attendancy condition 3 - absent % is over 12%'] === 'yes') {{
                reasons.push('${{translations.orgChartModal?.nonPaymentReasons?.absenceRate12?.[lang] || '결근율 12% 초과 (출근 조건 3번 미충족)'}}');
            }}
            if (employee['attendancy condition 4 - minimum working days'] === 'yes') {{
                reasons.push('${{translations.orgChartModal?.nonPaymentReasons?.minWorkingDays?.[lang] || '최소 근무일 미달 (출근 조건 4번 미충족)'}}');
            }}
            ...
            if (position.includes('LINE') && position.includes('LEADER')) {{
                if (employee['aql condition 7 - team/area fail AQL'] === 'yes') {{
                    reasons.push('${{translations.orgChartModal?.nonPaymentReasons?.teamAreaAQL?.[lang] || '팀/구역 AQL 실패 (AQL 조건 7번 미충족)'}}');
                }}
            ...
            if (employee['5prs condition 1 - there is  enough 5 prs validation qty or pass rate is over 95%'] === 'no') {{
                reasons.push('${{translations.orgChartModal?.nonPaymentReasons?.fprsPassRate?.[lang] || '5PRS 검증 부족 또는 합격률 95% 미달 (5PRS 조건 1번 미충족)'}}');
            }}
            if (employee['5prs condition 2 - Total Valiation Qty is zero'] === 'yes') {{
                reasons.push('${{translations.orgChartModal?.nonPaymentReasons?.fprsZeroQty?.[lang] || '5PRS 총 검증 수량 0 (5PRS 조건 2번 미충족)'}}');
            }}
        ...
        window.showIncentiveModal = function(nodeId) {{
            ...
            try {{
                ...
                // ${{translations.orgChartModal?.calculationDetails?.[lang] || tr('orgChartModal.calculationDetails', '계산 과정 상세')}} 내용 생성
                ...
                if (assemblyInspectors.length > 0) {{
                    inspectorDetails = `
                        <div class="mt-3">
                            <h6>📋 ${{translations.orgChartModal?.assemblyInspectorDetails?.[lang] || 'ASSEMBLY INSPECTOR 인센티브 내역 (합계 계산 대상)'}}</h6>
                            <table class="table table-sm table-bordered">
                                <thead class="table-light">
                                    <tr>
                                        <th>${{translations.common?.tableHeaders?.name?.[lang] || tr('orgChartModal.name', '이름')}}</th>
                                        <th>ID</th>
                                        <th class="text-end">${{translations.orgChartModal?.incentive?.[lang] || tr('orgChartModal.incentive', '인센티브')}}</th>
                                        <th class="text-center">${{translations.orgChartModal?.receivingStatus?.[lang] || tr('orgChartModal.receivingStatus', '수령 여부')}}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                ...
                                <tfoot class="table-secondary">
                                    <tr>
                                        <th colspan="2">${{translations.orgChartModal?.total?.[lang] || tr('orgChartModal.total', '합계')}}</th>
                ...
                calculationDetails = `
                    <div class="calculation-details">
                        <h6>📊 ${{translations.orgChartModal?.calculationDetails?.[lang] || tr('orgChartModal.calculationDetails', '계산 과정 상세')}} (LINE LEADER)</h6>
                        <table class="table table-sm">
                            <tr>
                                <td>계산 공식:</td>
                            ...
                            <tr>
                                <td>${{translations.orgChartModal?.calculationFormula?.[lang] || tr('orgChartModal.calculationFormula', '계산식')}}:</td>
                ...
                if (teamLineLeaders.length > 0) {{
                    lineLeaderDetails = `
                        <div class="mt-3">
                            <h6>📋 <span class="modal-team-line-leader-list">${{translations.orgChartModal?.teamLineLeaderDetails?.[lang] || '팀 내 LINE LEADER 인센티브 내역 (평균 계산 대상)'}}</span></h6>
                            <table class="table table-sm table-bordered">
                                <thead class="table-light">
                                    <tr>
                                        <th>${{translations.common?.tableHeaders?.name?.[lang] || tr('orgChartModal.name', '이름')}}</th>
                                        <th>ID</th>
                                        <th class="text-end">${{translations.orgChartModal?.incentive?.[lang] || tr('orgChartModal.incentive', '인센티브')}}</th>
                                        <th class="text-center">${{translations.orgChartModal?.includeInAverage?.[lang] || tr('orgChartModal.includeInAverage', '평균 계산 포함')}}</th>
                                    </tr>
                                </thead>
                                <tbody>
                ...
                calculationDetails = `
                    <div class="calculation-details">
                        <h6>📊 ${{translations.orgChartModal?.calculationDetails?.[lang] || tr('orgChartModal.calculationDetails', '계산 과정 상세')}} (GROUP LEADER)</h6>
                        <table class="table table-sm">
                            <tr>
                                <td>계산 공식:</td>
                            ...
                            <tr>
                                <td><span class="modal-team-line-leader-count">${{translations.orgChartModal?.teamLineLeaderCount?.[lang] || tr('orgChartModal.teamLineLeaderCount', '팀 내 LINE LEADER 수')}}:</span></td>
                                <td class="text-end">${{teamLineLeaders.length}}명 (수령: ${{receivingLineLeaders.length}}명)</td>
                            </tr>
                            <tr>
                                <td>${{translations.orgChartModal?.lineLeaderAverage?.[lang] || tr('orgChartModal.lineLeaderAverage', 'LINE LEADER 평균 인센티브')}}:</td>
                    ...
                    allLineLeaderDetails = `
                        <div class="mt-3">
                            ...
                            <table class="table table-sm table-bordered">
                                ...
                                <tfoot class="table-secondary">
                                    <tr>
                                        <th colspan="3">${{translations.orgChartModal?.total?.[lang] || tr('orgChartModal.total', '합계')}}</th>
                                        <th class="text-end">₫${{receivingLineLeaders.reduce((sum, ll) => sum + Number(ll[dashboardMonth + '_incentive'] || 0), 0).toLocaleString('ko-KR')}}</th>
                                        <th></th>
                                    </tr>
                                    <tr>
                ...
                calculationDetails = `
                    <div class="calculation-details">
                        <h6>📊 ${{translations.orgChartModal?.calculationDetails?.[lang] || tr('orgChartModal.calculationDetails', '계산 과정 상세')}} (SUPERVISOR)</h6>
                        <table class="table table-sm">
                            <tr>
                                <td>계산 공식:</td>
                ...
                calculationDetails = `
                    <div class="calculation-details">
                        <h6>📊 ${{translations.orgChartModal?.calculationDetails?.[lang] || tr('orgChartModal.calculationDetails', '계산 과정 상세')}} (A.MANAGER)</h6>
                        <table class="table table-sm">
                            <tr>
                                <td>계산 공식:</td>
                ...
                calculationDetails = `
                    <div class="calculation-details">
                        <h6>📊 ${{translations.orgChartModal?.calculationDetails?.[lang] || tr('orgChartModal.calculationDetails', '계산 과정 상세')}} (MANAGER)</h6>
                        <table class="table table-sm">
                            <tr>
                                <td><span class="modal-team-line-leader-count">${{translations.orgChartModal?.teamLineLeaderCount?.[lang] || tr('orgChartModal.teamLineLeaderCount', '팀 내 LINE LEADER 수')}}:</span></td>
                            ...
                            <tr>
                                <td>${{translations.orgChartModal?.lineLeadersReceiving?.[lang] || tr('orgChartModal.lineLeadersReceiving', '인센티브 받은 LINE LEADER')}}:</td>
                            ...
                            <tr class="table-warning">
                                <td><strong>${{translations.orgChartModal?.calculationFormula?.[lang] || tr('orgChartModal.calculationFormula', '계산식')}}:</strong></td>
                        ...
                        <div class="modal-content" style="z-index: 1057; position: relative; user-select: text; -webkit-user-select: text; -moz-user-select: text; -ms-user-select: text;">
                            ...
                            <div class="modal-body">
                                <div class="employee-info mb-3">
                                    <h5>${{employee.name}}</h5>
                                    <p class="mb-1"><strong>${{translations.orgChartModal?.position?.[lang] || tr('orgChartModal.position', '직급')}}:</strong> ${{employee.position}}</p>
            ...
            try {{
                ...
                node.append("rect")
                    ...
                    .attr("fill", d => {{
                        ...
                        // 인센티브 ${{translations.orgChartModal?.receivingStatus?.[lang] || tr('orgChartModal.receivingStatus', '수령 여부')}}에 따라 색상 조정
                        if (hasIncentive(d.data)) {{
