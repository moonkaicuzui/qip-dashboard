#!/usr/bin/env python3
"""
integrated_dashboard_final.py에 모달 개선사항을 영구적으로 적용
"""

import re

def apply_modal_improvements():
    """integrated_dashboard_final.py에 모달 함수 영구 적용"""
    ...
    pattern = r'(def generate_dashboard_html.*?)(    return html)'
    ...
    function showTotalWorkingDaysDetails() {
        const workDays = [2,3,4,5,6,9,10,11,12,13,16,17,18,19];
        const holidays = [1,7,8,14,15];
    ...
    # </script> 태그 앞에 모달 함수 추가
    script_pattern = r'(</script>)'

    # 모달 함수가 이미 있는지 확인
    if 'showTotalWorkingDaysDetails' not in content:
        # </script> 태그 앞에 삽입
        content = re.sub(
            script_pattern,
            modal_functions + r'\n\1',
        ...
        print("✅ 모달 함수가 integrated_dashboard_final.py에 추가되었습니다.")
    else:
        print("ℹ️ 모달 함수가 이미 존재합니다.")

    # 수정된 내용 저장
    with open('integrated_dashboard_final.py', 'w', encoding='utf-8') as f:
        f.write(content)

    print("\n✅ integrated_dashboard_final.py가 영구적으로 수정되었습니다!")
    print("📌 이제 action.sh를 실행해도 모달 개선사항이 유지됩니다.")
    return True

if __name__ == "__main__":
    apply_modal_improvements()