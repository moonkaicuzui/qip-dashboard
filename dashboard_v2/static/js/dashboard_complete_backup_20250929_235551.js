...
// - const dashboardYear
    ...
    function updateValidationKPIs(isInterimReport) {
        ...
        const kpiTotalWorkingDaysElement = document.getElementById('kpiTotalWorkingDays');
        if (kpiTotalWorkingDaysElement) {
            kpiTotalWorkingDaysElement.textContent = totalWorkingDays + '일';
        }
        ...
        const kpiAbsentElement = document.getElementById('kpiAbsentWithoutInform');
        if (kpiAbsentElement) {
            kpiAbsentElement.textContent = ar1Over3 + '명';
        }
    ...
    function updateTalentPoolSection() {
        const talentPoolSection = document.getElementById('talentPoolSection');
        if (!talentPoolSection) {
            console.log('talentPoolSection element not found - skipping updateTalentPoolSection');
            return;
        }
        ...
        if (talentPoolMembers.length > 0) {
            // Talent Pool 섹션 표시
            talentPoolSection.style.display = 'block';
    ...
    function generateEmployeeTable() {
        ...
        if (!tbody) {
            console.log('employeeTableBody element not found - skipping generateEmployeeTable');
            return;
        }
        tbody.innerHTML = '';
    ...
    function showPositionDetail(type, position) {
        ...
        // paidEmployees는 이미 line 9022에서 선언됨
        ...
        console.log('showPositionDetail - Type normalization:', type, '->', normalizedType);

        // TYPE-1의 경우 position에 따라 세분화된 조건 적용
        let applicableConditions = [];

        if (normalizedType === 'TYPE-1' || normalizedType === 'TYPE1') {
            ...
            if (positionUpper.includes('(V) SUPERVISOR') || positionUpper.includes('V.SUPERVISOR') || positionUpper.includes('V SUPERVISOR')) {
                ...
                // 기본 TYPE-1 (매칭되지 않는 경우 출근 조건만)
                applicableConditions = [1, 2, 3, 4];
            }
        } else if (normalizedType === 'TYPE-2' || normalizedType === 'TYPE2') {
            applicableConditions = [1, 2, 3, 4];  // TYPE-2는 출근 조건만
            console.log('TYPE-2 detected in showPositionDetail, conditions:', applicableConditions);
        } else if (normalizedType === 'TYPE-3' || normalizedType === 'TYPE3') {
            applicableConditions = [];  // TYPE-3는 조건 없음
            console.log('TYPE-3 detected in showPositionDetail, no conditions');
        }

        // conditionStats 초기화 - 적용 가능한 조건만
        ...
        console.log('Initialized conditionStats for', normalizedType, position, ':', conditionStats);
        console.log('conditionStats object keys:', Object.keys(conditionStats));
        console.log('conditionStats values:', Object.values(conditionStats));

        // 직원별 조건 평가 결과 계산
        console.log('Evaluating conditions for', employees.length, 'employees of type', normalizedType, 'position', position);
        console.log('Applicable conditions:', applicableConditions);

        // 모든 직원에 대해 조건을 평가하기 전에 데이터 확인
        if (employees.length > 0) {
            const sampleEmp = employees[0];
            console.log('Sample employee:', sampleEmp.name);
            console.log('Key fields:', {
                'Absence Rate (raw)': sampleEmp['Absence Rate (raw)'],
                'Unapproved Absences': sampleEmp['Unapproved Absences'],
                'Actual Working Days': sampleEmp['Actual Working Days'],
                'Total Working Days': sampleEmp['Total Working Days'],
                'condition_results': sampleEmp.condition_results?.length || 0
            });
        }
        ...
        employees.forEach((emp, empIndex) => {
            ...
            if (emp.condition_results && Array.isArray(emp.condition_results) && emp.condition_results.length > 0) {
                ...
                // TYPE-2와 TYPE-1 모두 여기서 처리
                applicableConditions.forEach(condId => {
                    // 조건별 개별 평가 - 실제 필드명 사용
                    switch(condId) {
                        case 1: // 출근율 ≥88% (TYPE-1) 또는 ≥96% (TYPE-2)
                            const attendanceThreshold = (normalizedType === 'TYPE-1' || normalizedType === 'TYPE1') ? 88 : 96;
                            // Use mapped fields from DataProcessor and cond_* fields
                            const attendanceRate = parseFloat(emp['attendance_rate']) || parseFloat(emp['cond_1_value']) || 0;
                            ...
                            if (attendanceRate > 0) {
                                ...
                                // 데이터가 없으면 기본값 처리
                                conditionStats[1].total++;
                                conditionStats[1].met++; // 결근율 0%는 출근율 100%
                            }
                            break;

                        case 2: // 무단결근 2일 이하
                            const unapproved = parseInt(emp['cond_2_value']) || parseInt(emp['Unapproved Absences']) || 0;
                        ...
                        case 3: // 실제근무일 0일 초과
                            const workdays = parseInt(emp['cond_3_value']) || parseInt(emp['Actual Working Days']) || 0;
                        ...
                        case 4: // 최소 근무일: 전체 근무일의 절반 이상
                            const actualDays = parseInt(emp['cond_3_value']) || parseInt(emp['Actual Working Days']) || 0;
                            const totalDays = parseInt(emp['cond_4_value']) || parseInt(emp['Total Working Days']) || 21;
                            conditionStats[4].total++;
                            const minRequired = parseInt(emp['cond_4_threshold']) || 12;
                            if (actualDays >= minRequired) {
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="overflow-x: auto;">
                        <table class="table table-sm" style="font-size: 0.9rem;">
                            ...
                            <tbody>
                                ${(() => {
                                    console.log('Rendering condition table. conditionStats:', conditionStats);
                                    console.log('conditionStats entries:', Object.entries(conditionStats));
                                    return Object.entries(conditionStats).map(([id, stat], index) => {
                        ...
                        <div style="display: flex; gap: 5px; flex-wrap: wrap;">
                            ${(() => {
                                ...
                                if (emp.condition_results && emp.condition_results.length > 0) {
                                    ...
                                    // TYPE-2의 경우 출근 조건만, TYPE-1은 모든 조건
                                    const normalizedEmpType = String(type).toUpperCase().trim();

                                    // 출근율 체크 - Absence Rate (raw)에서 계산
                                    const absenceRateField = emp['Absence Rate (raw)'];
                                    let hasAttendanceData = false;
                                    let attendanceMet = true;

                                    if (absenceRateField !== undefined && absenceRateField !== '' && absenceRateField !== null) {
                                        hasAttendanceData = true;
                                        const absenceRate = parseFloat(String(absenceRateField).replace('%', ''));
                                        const attendanceRate = 100 - absenceRate; // 출근율 = 100 - 결근율
                                        const threshold = normalizedEmpType === 'TYPE-1' ? 88 : 96;
                                        if (attendanceRate < threshold) attendanceMet = false;
                                    }

                                    // 무단결근 체크
                                    const unapprovedField = emp['Unapproved Absences'];
                                    if (unapprovedField !== undefined && unapprovedField !== '' && unapprovedField !== null) {
                                        hasAttendanceData = true;
                                        const unapproved = parseInt(unapprovedField) || 0;
                                        if (unapproved > 2) attendanceMet = false;
                                    }

                                    // 실제 근무일 체크
                                    const actualDaysField = emp['Actual Working Days'];
                                    const totalDaysField = emp['Total Working Days'] || 21;
                                    if (actualDaysField !== undefined && actualDaysField !== '' && actualDaysField !== null) {
                                        hasAttendanceData = true;
                                        const actualDays = parseInt(actualDaysField) || 0;
                                        const totalDays = parseInt(totalDaysField) || 21;
                                        if (actualDays <= 0 || actualDays < Math.ceil(totalDays / 2)) attendanceMet = false;
                                    }
                                    ...
                                    // TYPE-1만 AQL과 5PRS 체크
                                    if (normalizedEmpType === 'TYPE-1') {
                                        // AQL 체크 - 실제 데이터 필드 사용
                                        const aqlFailures = emp['September AQL Failures'] || emp[`${dashboardMonth.charAt(0).toUpperCase() + dashboardMonth.slice(1)} AQL Failures`];
                                        const aqlCondition = emp['cond_5_aql_personal_failure'];
                                        const aqlConsecutive = emp['cond_6_aql_continuous'];

                                        let aqlMet = true;
                                        let hasAqlData = false;

                                        if (aqlCondition !== undefined && aqlCondition !== '' && aqlCondition !== null && aqlCondition !== 'N/A') {
                                            hasAqlData = true;
                                            if (aqlCondition === 'FAIL') aqlMet = false;
                                        } else if (aqlFailures !== undefined && aqlFailures !== '' && aqlFailures !== null) {
                                            hasAqlData = true;
                                            if (aqlFailures > 0) aqlMet = false;
                                        }

                                        if (aqlConsecutive !== undefined && aqlConsecutive !== '' && aqlConsecutive !== null && aqlConsecutive !== 'N/A') {
                                            hasAqlData = true;
                                            if (aqlConsecutive === 'FAIL') aqlMet = false;
                                        }
                                        ...
                                        // 5PRS 체크
                                        const prsScoreField = emp['5PRS_Pass_Rate'] || emp['Average 5PRS score'] || emp['5PRS score'];
                                        const prsVolumeField = emp['5PRS_Inspection_Qty'] || emp['5PRS_volume'];

                                        let prsMet = true;
                                        let hasPrsData = false;

                                        if (prsScoreField !== undefined && prsScoreField !== '' && prsScoreField !== null) {
                                            hasPrsData = true;
                                            const prsScore = parseFloat(String(prsScoreField).replace('%', ''));
                                            if (prsScore < 95) prsMet = false;
                                        }

                                        if (prsVolumeField !== undefined && prsVolumeField !== '' && prsVolumeField !== null) {
                                            hasPrsData = true;
                                            const prsVolume = parseInt(prsVolumeField) || 0;
                                            if (prsVolume < 100) prsMet = false;
                                        }
    ...
    function updatePositionFilter() {
        ...
        if (!typeFilterElement || !positionSelect) {
            console.log('Filter elements not found - skipping updatePositionFilter');
            return;
        }
    ...
    function evaluateEmployeeConditions(employee) {
        // TYPE 필드 찾기 - 다양한 필드명 확인
        const type = employee.type ||
                    employee['ROLE TYPE STD'] ||
                    employee['Type'] ||
        ...
        const position = (employee.position ||
                         employee['FINAL QIP POSITION NAME CODE'] ||
                         employee['QIP POSITION 1ST  NAME'] ||
                         employee['Position'] ||
                         employee['직급'] ||
                         '').toUpperCase();

        // 디버깅 로그 개선
        console.log('=== Evaluating Employee Conditions ===');
        console.log('Employee:', employee.name || employee['Full Name']);
        console.log('Type detected:', type);
        console.log('Position detected:', position);
        console.log('All fields:', Object.keys(employee).slice(0, 20));
        console.log('Employee data sample:', {
            'Absence Rate (raw)': employee['Absence Rate (raw)'],
            'Unapproved Absences': employee['Unapproved Absences'],
            'Actual Working Days': employee['Actual Working Days']
        });
        ...
        if (normalizedType === 'TYPE-1' || normalizedType === 'TYPE1') {
            ...
            applicableConditions = [1, 2, 3, 4];
            console.log('TYPE-2 detected, conditions:', applicableConditions);
        } else if (normalizedType === 'TYPE-3' || normalizedType === 'TYPE3') {
            applicableConditions = [];
            console.log('TYPE-3 detected, no conditions');
        }

        // 조건 평가 결과 생성
        const results = [];
        const conditionDefinitions = {
            1: { name: '출근율', threshold: type === 'TYPE-1' ? '≥88%' : '≥96%', unit: '%' },
            2: { name: '무단결근', threshold: '≤2일', unit: '일' },
            3: { name: '실제 근무일', threshold: '>0일', unit: '일' },
            4: { name: '최소 근무일', threshold: '≥전체의 절반', unit: '일' },
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            // 조건별 평가 - Excel 필드명과 일치
            switch (i) {
                case 1: // 출근율 ≥88% (TYPE-1) 또는 ≥96% (TYPE-2)
                    const threshold = (normalizedType === 'TYPE-1' || normalizedType === 'TYPE1') ? 88 : 96;
                    // Use mapped fields and cond_1_value
                    value = parseFloat(employee['attendance_rate']) || parseFloat(employee['cond_1_value']) || 100;
                    met = value >= threshold;
                    break;

                case 2: // 무단결근 ≤2일
                    value = parseInt(employee['cond_2_value']) || parseInt(employee['Unapproved Absences']) || 0;
                    met = value <= 2;
                    break;

                case 3: // 실제 근무일 >0일
                    value = parseInt(employee['cond_3_value']) || parseInt(employee['Actual Working Days']) || 0;
                    met = value > 0;
                    break;

                case 4: // 최소 근무일 (전체 근무일의 절반 이상)
                    const actualDays = parseInt(employee['cond_3_value']) || parseInt(employee['Actual Working Days']) || 0;
                    const totalDays = parseInt(employee['cond_4_value']) || parseInt(employee['Total Working Days']) || 21;
                    const minRequired = parseInt(employee['cond_4_threshold']) || 12;
                    value = actualDays;
                    met = actualDays >= minRequired;
                    break;

                case 5: // 당월 AQL 0건
                    // Use cond_5_value for AQL failures
                    value = parseInt(employee['cond_5_value']) || parseInt(employee['September AQL Failures']) || 0;
                    met = value === 0;
                    break;

                case 6: // 3개월 연속 AQL
                    // cond_6_aql_continuous 필드 확인
                    const cond6Result = employee['cond_6_aql_continuous'];
                    if (cond6Result === 'PASS') {
                        value = 0; // PASS면 연속 실패 0
                        met = true;
                    } else if (cond6Result === 'FAIL') {
                        value = 3; // FAIL면 3개월 이상 연속 실패
                        met = false;
                    } else {
                        // 기본값
                ...
                case 7: // 팀/구역 AQL ≤5%
                    value = parseFloat(employee['team_aql_fail_rate'] || 0) * 100;
                    met = value <= 5;
                    break;

                case 8: // 담당구역 Reject ≤2%
                    value = parseFloat(employee['area_reject_rate'] || 0) * 100;
                    met = value <= 2;
                    break;

                case 9: // 5PRS 통과율 ≥95%
                    // 다양한 필드명 지원
                    value = parseFloat(employee['5PRS_Pass_Rate'] || employee['5PRS Pass Rate'] || employee['pass_rate'] || 0);
                    met = value >= 95;
                    break;

                case 10: // 5PRS 검사량 ≥100족
                    // 다양한 필드명 지원
                    value = parseInt(employee['5PRS_Inspection_Qty'] || employee['5PRS Inspection Quantity'] || employee['validation_qty'] || 0);
                    met = value >= 100;
                    break;
            }
